...
public class PloomesConnection
{
    ...
    {
        Headers = new Dictionary<string, string>() { { "User-Key", deafultConnectionWorkSheet.Cells[CellDefaultPositions.USER_KEY_CELL].GetCellValue<string>() } };
    }

    public void SetEntity(ExcelWorksheet worksheet)
    {
        string entityFullName = worksheet.Cells["Z5"].GetCellValue<string>();
